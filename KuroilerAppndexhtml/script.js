...
function loadProducts() {
  ...
  products.forEach(product => {
    ...
    <div class="card">
      ...
      <button onclick="window.open('https://wa.me/${BUSINESS_PHONE}?text=${encodeURIComponent('Hi, I want to order ' + product.name + ' (₹' + product.price + ' per ' + product.unit + ')')}')">
        Order Now
      </button>
    </div>`;
  });
...
function loadOrders() {
  ...
  orders.forEach((o, index) => {
    totalSales += o.total;
    html += `<li>${index+1}. ${o.customerName} - ${o.product} - ${o.quantity} ${o.unit} - ₹${o.total}</li>`;
  });
